...
| [Tack](Tack/) | A small creature that shows you what is still held together with pins — the tree is ten separate git repositories and nothing answers for all of them at once. Tack keeps no list and re-reads every repo every time it is asked, so it cannot report something stale | Working: `node Tack/tack.js`, or double-click `Tack/Look.bat`. Ten repos swept, with age, a **live** flag when another session may be mid-write, and *no commits yet* as its own state. **Pass 1 is read-only and mechanized** — `READ_ONLY_VERBS` is a single choke point the selftest asserts by value (92 assertions, 18 mutants all caught). Staging, committing and restoring are pass 2, deliberately separate. Read its `CLAUDE.md` |
