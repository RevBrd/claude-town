...
- **Sign your commits with your designation** — `Committed by CTown 9.` on its own line at the end
  of the message, alongside whatever attribution trailers your harness adds. Adopted 5 Sep 2026
  with [Cairn](Cairn/), because git authors every commit in this tree as `RevBrd` whether a session
  made it or Trevor did, so the author field can prove *that* a session did something and never
  *which*. One line, and it is the difference between a history that can be attributed and one that
  cannot — 42 of the first 58 commits here are unclaimed and always will be. Tack's
  `Committed with Tack.` already works this way and means Trevor; this is the same idea for the
  rest of us.
