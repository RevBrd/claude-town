...
Marquee/
  ...
  tools/icon.js      prints the sign -- marquee.ico, seven sizes, no dependencies
  tools/shortcut.ps1 puts Marquee in the Start menu (and -Remove takes it out)
  marquee.ico        generated by icon.js, and committed -- see below
...
### The sign, and getting in without a folder
...
**The icon is generated, not drawn.** `tools/icon.js` writes `marquee.ico` — a projecting canopy,
a row of bulbs under its lip, and the reader board below — at seven sizes, with no image library
and no `npm install`. A PNG is a zlib stream with four chunks and a CRC, and node has zlib; an
`.ico` is a six-byte header, a directory, and the PNGs.

That is the same argument as the posters: **a binary nobody can regenerate is a binary nobody can
change.** And the palette is read out of `marquee.html`'s tunables block rather than retyped, so
the sign and the page cannot drift into two different houses. A missing variable throws by name
rather than defaulting quietly — a sign in the wrong colours should be loud.

Two details carried over from the facade on purpose. **Two bulbs are out and they stay out**, for
the reason the page gives: a sign with every bulb lit is a rendering, not a sign. And *which* two
is fixed rather than random, so a rebuild is byte-identical and not a diff.

**Detail is dropped by size rather than scaled into mud.** At 16px there are four bulbs and no
lettering; at 48 there are nine and the board has two lines on it. The first draft skipped the
canopy's rim and at 16px the whole silhouette vanished into the background — a row of bulbs
floating over nothing. Caught by rendering a magnified contact sheet of every small size and
looking at it, which is the only way to judge this: **a silhouette that fails only at 16px fails
exactly where an icon is actually looked at.**

**`marquee.ico` is committed**, even though it is generated. Same reasoning as `manifest.js`: the
Electron window and the Start menu shortcut both need it on disk and there is no build step here
to put it there. A generated file something reads at runtime is part of the deliverable.

**`tools/shortcut.ps1` puts Marquee in the Start menu** — one file, in the current user's own
Start menu folder. Nothing system-wide, nothing in the registry, no file associations, no
administrator. `-Remove` deletes exactly that one file, so it is undone by the same script that
did it. It points at `electron.exe` directly rather than at `Marquee.bat`, because a `.bat` goes
through `cmd` and flashes a console on the way past, and it **refuses to write a shortcut to
something that is not there** — a Start menu entry that opens an error is worse than no entry,
because it looks installed.

### The window remembers where it was

Its bounds go in Electron's `userData` folder, **never in this repo**: they are a fact about one
person's monitors, committing them would put a second author's window position into everybody's
checkout, and Tack would report the file as loose work after every single run.

**A remembered position is not automatically a reachable one.** Unplug the second monitor and last
night's window is off the edge of the world, opening somewhere the mouse cannot go — which looks
exactly like the app failing to start rather than like a stale rectangle. So a saved rectangle has
to still overlap a display that exists *now*, by at least 80px in both axes, and anything else
falls back to the default. `usableBounds()` is pure, so that is asserted against fabricated
one- and two-monitor setups rather than needing a second monitor to hand.

Fullscreen and maximised bounds are never saved. Restoring into fullscreen, with no menu bar and
no title bar, is a room with the door painted over.

### F11 and the zoom trio

`F11` toggles fullscreen; `Ctrl +`, `Ctrl -` and `Ctrl 0` zoom. Both follow the rule the double
Escape established: **they do not take a key away from any work.** F11 is the browser's fullscreen
key rather than anything a game reaches for, and the zoom keys are Ctrl-modified — a bare `-` or
`0` still belongs to whatever is running, which is asserted, because a game where you cannot type
a minus sign would be a strange thing to ship.

Zoom is the window's rather than the page's, so it scales a running work along with the lobby,
which is the point on a large monitor. It is clamped at both ends: a zoom level nobody can read is
a window nobody can fix without deleting a file they do not know exists.
...
64 pure assertions and 14 live ones. **One file, two runtimes**: run under node it asserts the
